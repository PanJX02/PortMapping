...
这是一个简单易用的Linux服务器端口转发工具，专为UDP协议服务（如Hysteria2）设计。它允许您将服务器上的多个UDP端口范围映射到指定的服务端口，实现端口跳跃功能。特别适合为V2bX面板和PPanel-node的Hysteria2节点配置端口跳跃，通过简单的命令行界面，您可以快速配置和管理多条端口映射规则，支持主流Linux发行版，并提供自动持久化功能。
...
1. 添加端口映射 - 可以添加多条映射规则
2. 取消端口映射 - 可以选择性删除单条规则或全部删除
3. 查看当前映射状态 - 显示所有映射规则详情
4. 检查更新
5. 显示版本信息
6. 卸载VPN端口映射工具
0. 退出
...
# 添加第二条映射规则（将外部30000-40000端口映射到Hysteria2的443端口）
sudo vpn 443 30000 40000
...
- **多条映射支持**：支持添加和管理多条端口映射规则
- **选择性删除**：可以选择性删除单条规则或一次性删除所有规则
- **规则ID管理**：每条规则都有唯一ID，便于精确管理
- **端口跳跃**：支持将端口范围映射到单个Hysteria2服务端口
- **一键配置**：通过简单命令快速设置端口跳跃规则
- **状态监控**：实时查看所有端口映射配置和状态
...
```bash
# 第一条规则：映射10000-20000到443
sudo vpn 443 10000 20000

# 第二条规则：映射30000-40000到443
sudo vpn 443 30000 40000

# 第三条规则：映射50000-60000到8080
sudo vpn 8080 50000 60000
```
...
```bash
# 如果已有规则 10000-20000 -> 443
# 尝试添加 15000-25000 -> 443 会失败并提示重叠
sudo vpn 443 15000 25000
# 输出：警告: 端口范围 15000-25000 与现有规则 10000-20000 重叠
```
